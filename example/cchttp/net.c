...
peer_conn_handler(TSCont contp, TSEvent event, void *edata)
{
  ...
  if (event == EC_EVENT_PEER_BEGIN_WRITE)
    TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %" PRId64 " peer %d receives EC_EVENT_PEER_BEGIN_WRITE", txn_id, peer->index);
  else
    TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %" PRId64 " peer %d receives %s", txn_id, peer->index,
            TSHttpEventNameLookup(event));

  switch (event) {
  case TS_EVENT_ERROR:
    TSError("[%s] peer_conn_handler: txn %ld peer %d ERROR", PLUGIN_NAME, txn_id, peer->index);
  ...
  case TS_EVENT_NET_CONNECT:
    ...
    TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %" PRId64 " peer %d (%s) Net Connect, %d peers have connected", txn_id,
  ...
  case EC_EVENT_PEER_BEGIN_WRITE:
    ...
    // TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %"PRId64 " peer %d, begin write to peer", txn_id, peer->index);
    break;
  ...
  case TS_EVENT_VCONN_WRITE_READY:
    TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %ld peer %d Write Ready (Done: %" PRId64 " Todo: %" PRId64 ")", txn_id,
            peer->index, TSVIONDoneGet(pcd->write_vio), TSVIONTodoGet(pcd->write_vio));
  ...
  case TS_EVENT_VCONN_READ_COMPLETE:
    ...
    txn_data = ssn_data->current_txn_data;

    // if ((txn_data->ready_peers & (1 << (peer->index))) != 0) {
    //   TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %" PRId64 " peer %d, receive TS_EVENT_VCONN_READ_COMPLETE, but peer is ready",
    //           txn_id, peer->index);
    //   return TS_SUCCESS;
    // }
    // else{
    ...
    // int64_t ready_peers = __sync_fetch_and_xor(&(txn_data->ready_peers), (1 << peer->index));
    int64_t ready_peers = __sync_or_and_fetch(&(txn_data->ready_peers), (1 << peer->index));

    TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %" PRId64 " peer %d receive all response, %d available, ready_peers %s", txn_id,
            peer->index, n_available, int64_to_bitstring_static(ready_peers));
    ...
    if (n_available == EC_k + EC_x - 1) {
      ...
      TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %ld peer %d response is not needed", txn_id, peer->index);
    }
  ...
  case TS_EVENT_VCONN_READ_READY:
    // TSDebug(PLUGIN_NAME, "peer_conn_handler: peer %d Read Ready", peer->index);
    {
      TxnData *txn_data = ssn_data->current_txn_data;
      ...
      if (available > 0) {
        ...
        while (n_left > 0) {
          ...
          TSDebug(PLUGIN_NAME, "avail %ld, n_read %ld read_vio n_done %ld, n_todo %ld n_bytes %ld, low watermark %ld", available,
                  n_read, TSVIONDoneGet(pcd->read_vio), TSVIONTodoGet(pcd->read_vio), TSVIONBytesGet(pcd->read_vio),
                  TSIOBufferWaterMarkGet(pcd->response_buffer));
          n_left = available - n_read;
          if (MY_DEBUG_LEVEL > 3) {
            char *temp = TSstrndup(response_str, n_read);
            TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %ld peer %d received %s", txn_id, peer->index, temp);
            TSfree(temp);
          }
          if (txn_data->peer_resp_buf[peer->index] == NULL) {
            ...
            if (cp && cp - response_str + 15 < n_read) {
              ...
              TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %ld peer %d find and allocate size %" PRId64 " buffer for peer response",
                      txn_id, peer->index, pcd->content_length);
            } else {
              // char *temp = TSstrndup(response_str, n_read);
              TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %ld peer %d didn't find Content-Length", txn_id, peer->index);
              // TSfree(temp);
            }
          }
          ...
          if (txn_data->peer_resp_buf[peer->index] != NULL) {
            ...
            if (pcd->content_has_began && n_read > 0) {
              ...
              TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %ld peer %d copy size %" PRId64 " content", txn_id, peer->index, n_read);
              pcd->read_in_length += n_read;
            } else {
              if (n_read > 0) {
                char *temp = TSstrndup(response_str, n_read);
                TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %ld peer %d content has began %d, n_read %" PRId64 ", read in %s",
                        txn_id, peer->index, pcd->content_has_began, n_read, temp);
                TSfree(temp);
              } else if (n_read == 0)
                ;
              else {
                TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %ld peer %d ERROR n_read less than 0, n_read %" PRId64, txn_id,
                        peer->index, n_read);
                response_str = TSIOBufferBlockReadStart(block, pcd->response_buffer_reader, &n_read);
                char *temp   = TSstrndup(response_str, n_read);
                TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %ld peer %d n_read %" PRId64 ", read in %s", txn_id, peer->index,
                        n_read, temp);
                TSfree(temp);
              }
            }
      ...
      if (pcd->content_length != 0 && pcd->content_length == pcd->read_in_length) {
        ...
        TSDebug(PLUGIN_NAME,
                "peer_conn_handler: txn %ld peer %d read error, read in more than available %" PRId64 " Bytes/%" PRId64 " Bytes",
                txn_id, peer->index, pcd->read_in_length, pcd->content_length);
      } else {
  ...
  case TS_EVENT_VCONN_INACTIVITY_TIMEOUT:
    ...
    break;
  default:
    TSDebug(PLUGIN_NAME, "peer_conn_handler: txn %ld peer %d unknown TS_EVENT %s", txn_id, peer->index,
            TSHttpEventNameLookup(event));
...
{
  TxnData *txn_data = TSHttpTxnArgGet(txnp, txn_data_ind);
  TSDebug(PLUGIN_NAME, "EC_K_back: txn %" PRId64 " K+X-1 resp back! available peers %d %s", TSHttpTxnIdGet(txnp),
  ...
  for (int i = 0; i < EC_n; i++) {
    ...
    if (current_peer_available > 0) {
      ...
      TSDebug(PLUGIN_NAME, "EC_K_back: txn %" PRId64 " response %d %s", TSHttpTxnIdGet(txnp), i, txn_data->peer_resp_buf[i]);
    }
  }
  TSAssert(available_peers >= EC_k - 1);
  TSDebug(PLUGIN_NAME, "EC_K_back: K %d X %d, response size sum is %" PRId64, EC_k, EC_x, final_size);
  ...
  TSDebug(PLUGIN_NAME, "EC_K_back: txn %" PRId64 " final resp %s", TSHttpTxnIdGet(txnp), txn_data->final_resp);
