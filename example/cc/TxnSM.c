...
main_handler(TSCont contp, TSEvent event, void *data) {
  ...
  // TSDebug(PLUGIN_NAME, "main_handler (contp %p event %d data %p), possible events: error (%d) TS_EVENT_VCONN_EOS (%d)", contp, event, data,
  //         TS_EVENT_ERROR, TS_EVENT_VCONN_EOS);

  // TSDebug(PLUGIN_NAME,
  //         "current_handler (%p), state_dns_lookup (%p)",
  //         q_current_handler, state_dns_lookup);

  /* handle common cases errors */
  if (event == TS_EVENT_ERROR) {
    TSDebug(PLUGIN_NAME, "main handler get event error");
    return prepare_to_die(contp);
  }

  // if (q_current_handler != (TxnSMHandler)&state_interface_with_server) {
  //   if (event == TS_EVENT_VCONN_EOS) {
  //     TSDebug(PLUGIN_NAME, "q_current_handler q_current_handler (%p) != state_interface_with_server(%p)", q_current_handler,
...
state_start(TSCont contp, TSEvent event ATS_UNUSED, void *data ATS_UNUSED) {
  ...
  txn_sm->q_client_read_vio = TSVConnRead(txn_sm->q_client_vc, (TSCont) contp, txn_sm->q_client_request_buffer,
                                          INT64_MAX);
    ...
    case TS_EVENT_VCONN_READ_READY:
      ...
      if (bytes_read > 0) {
        ...
        if (strstr(txn_sm->q_client_request, "\r\n\r\n") != NULL) {
          ...
          // Jason: Debug HTTP
          sprintf(temp_buf, "GET %s HTTP/1.1\r\nHost: %s:%d\r\n\r\n", txn_sm->q_file_name, txn_sm->q_server_name,
                  txn_sm->q_server_port);
          strcpy(txn_sm->q_client_request, temp_buf);
          TSDebug(PLUGIN_NAME, "update client request to %s", txn_sm->q_client_request);
          ...
          temp_buf = (char *)TSmalloc(sizeof(char) * (2400));
          memset(temp_buf, 0, 2400);
          // http://d30.jasony.me:8080/ab
          // sprintf(temp_buf, "http://d30.jasony.me:8080%s", txn_sm->q_file_name);
          sprintf(temp_buf, "http://%s:%d/%s", txn_sm->q_server_name, txn_sm->q_server_port, txn_sm->q_file_name);

          // TSDebug(PLUGIN_NAME, "tempbuf len %lu %s, server_name len %lu %s", strlen(temp_buf), temp_buf,
          //         strlen(txn_sm->q_server_name), txn_sm->q_server_name);
          // TSstrlcpy(temp_buf, "http://", 8);
          // TSstrlcat(temp_buf, txn_sm->q_server_name, 2400);
          // TSDebug(PLUGIN_NAME, "tempbuf len %lu %s", strlen(temp_buf), temp_buf);
          // TSstrlcat(temp_buf, txn_sm->q_file_name, 2400);
          TSDebug(PLUGIN_NAME, "tempbuf len %lu %s", strlen(temp_buf), temp_buf);

          TSDebug(PLUGIN_NAME, "Key material: server_name %s, file_name %s, lookup key is %s*****", txn_sm->q_server_name, txn_sm->q_file_name, temp_buf);
          txn_sm->q_key = (TSCacheKey) CacheKeyCreate(temp_buf);
...
state_handle_cache_lookup(TSCont contp, TSEvent event, TSVConn vc) {
  ...
  switch (event) {
    case TS_EVENT_CACHE_OPEN_READ:
      ...
      TSDebug(PLUGIN_NAME, "cache hit! Response size %ld", response_size);
      ...
      txn_sm->q_cache_read_vio = TSVConnRead(txn_sm->q_cache_vc, contp, txn_sm->q_cache_read_buffer,
                                             response_size);
    ...
    case TS_EVENT_CACHE_OPEN_READ_FAILED:
      ...
      // TSDebug(PLUGIN_NAME, "state_handle_cache_lookup end");
...
state_handle_cache_read_response(TSCont contp, TSEvent event, TSVIO vio ATS_UNUSED) {
  ...
  switch (event) {
    case TS_EVENT_VCONN_READ_COMPLETE:
      TSDebug(PLUGIN_NAME, "TS_EVENT_VCONN_READ_COMPLETE, load and close");
    ...
    case TS_EVENT_VCONN_READ_READY:
      TSDebug(PLUGIN_NAME, "TS_EVENT_VCONN_READ_READY, load and re-enable");
      load_buffer_cache_data(txn_sm);
...
state_handle_cache_prepare_for_write(TSCont contp, TSEvent event, TSVConn vc) {
  ...
  switch (event) {
    case TS_EVENT_CACHE_OPEN_WRITE:
      ...
      break;
    default:
      TSDebug("[%s] Can't open cache write_vc, aborting txn", PLUGIN_NAME);
      txn_sm->q_cache_vc = NULL;
      return prepare_to_die(contp);
      break;
  }
...
state_build_and_send_request(TSCont contp, TSEvent event ATS_UNUSED, void *data ATS_UNUSED) {
  ...
  TSDebug(PLUGIN_NAME, "******** enter state_build_and_send_request, event %d", event);
  ...
  TSDebug(PLUGIN_NAME, "write %s to server request buffer", txn_sm->q_client_request);
  // Jason: Debug
//  char *tmp = "GET /abc HTTP/1.1\r\nHost: d30.jasony.me:2048\r\n\r\n";
//  TSIOBufferWrite(txn_sm->q_server_request_buffer, tmp, strlen(tmp));
//  TSDebug(PLUGIN_NAME, "write %s to server request buffer", tmp);
  ...
  TSDebug(PLUGIN_NAME, "initiating host lookup of %s", txn_sm->q_server_name);
...
state_dns_lookup(TSCont contp, TSEvent event, TSHostLookupResult host_info) {
  ...
  if (event != TS_EVENT_HOST_LOOKUP || !host_info) {
    TSDebug(PLUGIN_NAME, "can't find server ip or error, event %d (TS_EVENT_HOST_LOOKUP %d), host_info %p\n", event,
            TS_EVENT_HOST_LOOKUP, host_info);
    return prepare_to_die(contp);
  }
  txn_sm->q_pending_action = NULL;
  ...
  // Jason: Debug
  // int new_ip = (127 << 24) | (0 << 16) | (0 << 8) | (1);
  // new_ip = (128 << 24) | (2 << 16) | (208 << 8) | (240);
  // memset(&ip_addr, 0, sizeof(ip_addr));
  // ip_addr.sin_family = AF_INET;
  // ip_addr.sin_addr.s_addr = htonl(new_ip); /* Should be in network byte order */
  ip_addr.sin_port = htons(txn_sm->q_server_port);      // server_port;
  ...
  if (TSActionDone(txn_sm->q_pending_action)) {
...
state_connect_to_server(TSCont contp, TSEvent event, TSVConn vc) {
  ...
  if (event != TS_EVENT_NET_CONNECT) {
    TSDebug(PLUGIN_NAME, "TSNet connect to server failed");
    return prepare_to_die(contp);
  }
  txn_sm->q_pending_action = NULL;
  ...
  txn_sm->q_server_write_vio =
      TSVConnWrite(txn_sm->q_server_vc, contp, txn_sm->q_server_request_buffer_reader,
                   strlen(txn_sm->q_client_request));

  TSDebug(PLUGIN_NAME, "done initiating writing to server");
  return TS_SUCCESS;
}
...
state_send_request_to_server(TSCont contp, TSEvent event, TSVIO vio) {
  ...
  switch (event) {
    case TS_EVENT_VCONN_WRITE_READY:
      TSDebug(PLUGIN_NAME, "VCONN_WRITE_READY");
      TSVIOReenable(vio);
      break;
    case TS_EVENT_VCONN_WRITE_COMPLETE:
      TSDebug(PLUGIN_NAME, "VCONN_WRITE_COMPLETE");
      vio = NULL;

      /* Waiting for the incoming response. */
      set_handler(txn_sm->q_current_handler, (TxnSMHandler) &state_interface_with_server);
      txn_sm->q_server_read_vio = TSVConnRead(txn_sm->q_server_vc, contp, txn_sm->q_server_response_buffer,
                                              INT64_MAX);
      break;
...
state_interface_with_server(TSCont contp, TSEvent event, TSVIO vio) {
  ...
  switch (event) {
    ...
    case TS_EVENT_VCONN_WRITE_READY:
      TSDebug(PLUGIN_NAME, "state_interface_with_server: TS_EVENT_VCONN_WRITE_READY");
      return state_write_to_cache(contp, event, vio);

    case TS_EVENT_VCONN_WRITE_COMPLETE:
      TSDebug(PLUGIN_NAME, "state_interface_with_server: TS_EVENT_VCONN_WRITE_COMPLETE");
      return state_write_to_cache(contp, event, vio);
      /* Otherwise, handle events from server. */
    case TS_EVENT_VCONN_READ_READY:
      // TSDebug(PLUGIN_NAME, "state_interface_with_server: TS_EVENT_VCONN_READ_READY");
    ...
    case TS_EVENT_VCONN_READ_COMPLETE:
      TSDebug(PLUGIN_NAME, "state_interface_with_server: TS_EVENT_VCONN_READ_COMPLETE");
      return state_read_response_from_server(contp, event, vio);
    ...
    case TS_EVENT_VCONN_EOS:
      ...
      if (txn_sm->q_cache_response_length >= txn_sm->q_server_response_length) {
        ...
        TSDebug(PLUGIN_NAME, "cache_response_length is %d, server response length is %d",
...
state_read_response_from_server(TSCont contp, TSEvent event ATS_UNUSED, TSVIO vio ATS_UNUSED) {
  ...
  TSDebug(PLUGIN_NAME, "******** enter state_read_response_from_server, %d bytes available", bytes_read);

  // Jason: Debug
  // int64_t avail      = TSIOBufferReaderAvail(txn_sm->q_cache_response_buffer_reader);
  // TSIOBufferBlock bb = TSIOBufferReaderStart(txn_sm->q_cache_response_buffer_reader);
  // int64_t avail2     = TSIOBufferBlockReadAvail(bb, txn_sm->q_cache_response_buffer_reader);
  // int64_t avail3 = 0;
  // const char *dp     = TSIOBufferBlockReadStart(bb, txn_sm->q_cache_response_buffer_reader, &avail3);
  // TSDebug(PLUGIN_NAME, "avail %ld, %ld, %ld, server response %240s", avail, avail2, avail3, dp);
...
state_write_to_cache(TSCont contp, TSEvent event, TSVIO vio) {
  ...
  switch (event) {
    ...
    case TS_EVENT_VCONN_WRITE_COMPLETE:
      TSDebug(PLUGIN_NAME, "written to server, nbytes %"
      PRId64
      ", todo %"
      PRId64
      ", ndone %"
      PRId64, TSVIONBytesGet(vio),
          TSVIONTodoGet(vio), TSVIONDoneGet(vio));
      ...
      if (txn_sm->q_cache_response_length > 0 && txn_sm->q_block_bytes_read > 0) {
        TSDebug(PLUGIN_NAME, "consume %d bytes from q_cache_response_buffer_reader",
                txn_sm->q_block_bytes_read);
        TSIOBufferReaderConsume(txn_sm->q_cache_response_buffer_reader, txn_sm->q_block_bytes_read);
      }
      // Jason: Change
      //    if (TSVIONDoneGet(vio) > 0) {
      //      TSIOBufferReaderConsume(txn_sm->q_cache_response_buffer_reader, TSVIONDoneGet(vio));
      //      TSDebug(PLUGIN_NAME, "consume %ld bytes from q_cache_response_buffer_reader", TSVIONDoneGet(vio));
      //    }
      ...
      if (txn_sm->q_server_vc != NULL) {
        TSDebug(PLUGIN_NAME,
                "re-enable server_read_vio, cache_response_length is %d, server_response_length is %d",
      ...
      if (txn_sm->q_cache_response_length >= txn_sm->q_server_response_length) {
        ...
        TSDebug(PLUGIN_NAME,
                "re-enable cache_write_vio, cache_response_length is %d, server_response_length is %d",
                txn_sm->q_cache_response_length, txn_sm->q_server_response_length);
        TSVIOReenable(txn_sm->q_cache_write_vio);
      }
      return TS_SUCCESS;
...
state_send_response_to_client(TSCont contp, TSEvent event, TSVIO vio) {
  ...
  switch (event) {
    case TS_EVENT_VCONN_WRITE_READY:
      ...
      TSDebug(PLUGIN_NAME, "write_ready: nbytes %"
      PRId64
      ", ndone %"
      PRId64, TSVIONBytesGet(vio), TSVIONDoneGet(vio));
      TSVIOReenable(txn_sm->q_client_write_vio);
      break;

    case TS_EVENT_VCONN_WRITE_COMPLETE:
      TSDebug(PLUGIN_NAME, " . wr complete");
      TSDebug(PLUGIN_NAME, "write_complete: nbytes %"
      PRId64
      ", ndone %"
      PRId64, TSVIONBytesGet(vio), TSVIONDoneGet(vio));
...
parse_request(char *request, char *server_name, char *file_name) {
  ...
  TSDebug(PLUGIN_NAME, "serverName %s, filename %s\n", server_name, file_name);
  return 1;
}
